...
{
    BOT_ASSERT(m_bot->Map().isConnected(getTilePosition(), pos), ("Error: Build Position is not connected to worker (build pos:[" + std::to_string(pos.x) + ", " + std::to_string(pos.y) + "], unit pos:[" + std::to_string(getTilePosition().x) + ", " + std::to_string(getTilePosition().y) + "])").c_str());
    BOT_ASSERT(isValid(), "Unit is not valid");
#ifdef SC2API
