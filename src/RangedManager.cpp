...
void RangedManager::assignTargets(const std::vector<Unit> & targets)
{
    ...
    if (m_bot.Config().AlphaBetaPruning) {
        ...
        size_t depth = 6;
        AlphaBetaConsideringDurations alphaBeta = AlphaBetaConsideringDurations(std::chrono::milliseconds(40), depth, m_bot.Config().ClosestEnemy, m_bot.Config().WeakestEnemy, m_bot.Config().HighestPriority);
